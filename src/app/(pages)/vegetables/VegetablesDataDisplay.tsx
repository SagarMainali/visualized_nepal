...
type VegetableDataProps = {
    initialVegetableData: RangedDate_SingleVegetable_ForChartT[]
}

export default function VegetableDataDisplay({ initialVegetableData }: VegetableDataProps) {
    const [rangedDate_SingleVegetable, setRangedDate_SingleVegetable] = useState<RangedDate_SingleVegetable_ForChartT[]>(initialVegetableData);

    const [selectedVegetable, setSelectedVegetable] = useState('Tomato Big(Nepali)');

    // fetch next vegetable data
    const handleVegetableChange = async (nextVegetable: string) => {
        setSelectedVegetable(nextVegetable);

        try {
            const { data } = await axios.get(`/api/vegetables/${encodeURIComponent(nextVegetable)}`);
            setRangedDate_SingleVegetable(data);
        } catch (error) {
            console.error("Failed to fetch new vegetable data:", error);
        }
    };
            ...
            <div className='h-[94vh] w-full'>
                <div className='h-full w-full flex flex-col justify-center items-center gap-2'>
                    <div className='w-[85%] flex justify-end items-center gap-2'>
                        ...
                        <SearchableDropDown items={commodities} label={selectedVegetable} onClickHandler={handleVegetableChange} selectedValue={selectedVegetable} />
                    </div>
